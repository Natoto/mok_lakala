...
- (void)stopLoading {
    ...
    if ([mimeType isEqualToString:@"application/json"]) {
        ...
        if (xmlString && xmlString.length>0) {
            ne_HTTPModel.receiveJSONData = xmlString;
            //example http://webservice.webxml.com.cn/webservices/qqOnlineWebService.asmx/qqCheckOnline?qqCode=2121
        }
    }

//#if DEBUG
    if ([self.ne_HTTPModel.requestURLString containsString:@".jpg"] || [self.ne_HTTPModel.requestURLString containsString:@"/large/"] || [self.ne_HTTPModel.requestURLString containsString:@"pstatp.com/"]) {
    }
    else {
            NSLog(@"\n\n%@ %@ \n【body:%@】 \nresponse json:\n %@\n\n",self.ne_HTTPModel.requestHTTPMethod,self.ne_HTTPModel.requestURLString,self.ne_HTTPModel.requestHTTPBody,ne_HTTPModel.receiveJSONData);
            [[NSNotificationCenter defaultCenter] postNotificationName:@"notify_request_url" object:self.ne_HTTPModel.requestURLString];
    }
        //#endif
